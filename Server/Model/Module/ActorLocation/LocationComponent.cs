...
namespace ETModel
{
	...
	public class LocationComponent : Component
	{
		...
		public readonly HashSet<long> lockEventHashSet = new HashSet<long>();

		public readonly Dictionary<long, Queue<long>> lockEventDict = new Dictionary<long, Queue<long>>();
		...
		public async ETVoid Lock(long key, long instanceId, int time = 0)
		{
			...
			if (time > 0)
			{
				await Game.Scene.GetComponent<TimerComponent>().WaitForMilliSecondAsync(time);
		...
		public override void Dispose()
		{
			...
			this.lockEventDict.Clear();
			this.lockEventHashSet.Clear();
		}
	}
}