...
namespace ETModel
{
    ...
    public sealed class TChannel : AChannel
    {
            ...
            try
            {
                if (socket != null)
                {
                    if (socket.Connected)
                    {
                        ...
                        {
                            Log.Error("關閉 socket 失敗：" + ex.Message);
                        }
                    }
                    socket.Close();
                    socket.Dispose();
        ...
        public override void Send(MemoryStream stream)
        {
            if (this.IsDisposed)
            {
                throw new ObjectDisposedException("TChannel已经被Dispose, 不能发送消息");
            }
            ...
            switch (this.GetService().PacketSizeLength)
            {
                case Packet.PacketSizeLength4:
                    ...
                    {
                        throw new ArgumentOutOfRangeException($"send packet too large: {stream.Length}");
                    }
                ...
                case Packet.PacketSizeLength2:
                    ...
                    break;
                default:
                    throw new NotImplementedException("packet size must be 2 or 4!");
            }
        ...
        private void StartRecv()
        {
            ...
            IncreaseReceivedPackSize(size, 1);
        ...
        public void StartSend()
        {
            ...
            IncreaseSendedPackSize(sendSize, 1);
