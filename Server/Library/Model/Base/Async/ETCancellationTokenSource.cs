...
namespace ETModel
{
    ...
    public class ETCancellationTokenSource : Component
    {
        ...
        public ETCancellationTokenSource()
        {
            this.CancellationTokenSource = new CancellationTokenSource();
        }
        public ETCancellationTokenSource(long millisecondsDelay)
        {
            this.CancellationTokenSource = new CancellationTokenSource();
            this.CancelAfter(millisecondsDelay).Coroutine();
        }
        ...
        {
            await Game.Scene.GetComponent<TimerComponent>().WaitForMilliSecondAsync(afterTimeCancel);
            this.CancellationTokenSource?.Cancel();
            this.Dispose();
        }
